...
It is a custom WASM channel, not a normal tool. The baseline `0.2.10-baseline.1` clone proved that the package installs and activates as `CHANNEL` when installed with explicit `kind: "wasm_channel"`. The current `1.14` layer keeps built-in-style owner and pairing admission, persists canonical Simon family identities and per-recipient Telegram routing state for the install pack, and tells verified Telegram turns about the active Family Calendar read/write contract.
...
For the current `1.14` rebuild, use the public distribution bundle:
