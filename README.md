...
```text
bundles/simon_telegram_channel/1.8.tar.gz
```

After pushing tag `ironclaw-simon-telegram-1.8`, the direct install URL is:
...
Important: `1.8` keeps the `1.7` custom webhook route fix and adds hosted runtime/admission diagnostics. It should log `Simon Telegram channel runtime version 1.8` at startup and log redacted admission decisions before creating a pairing request or emitting an admitted message. It is still accepted for production only after hosted Telegram pairing and identity-continuity smoke tests pass.
...
```json
{
  ...
  "url": "https://raw.githubusercontent.com/rosenfeldalon/simon-ironclaw-extensions/ironclaw-simon-telegram-1.8/bundles/simon_telegram_channel/1.8.tar.gz",
  "kind": "wasm_channel"
}
```
...
IRONCLAW_SIMON_BUNDLE_VERSION=1.8 ./scripts/build-ironclaw-upload-bundles.sh
```
...
That lab found the `1.6` failure: the custom channel still used the built-in `/webhook/telegram` route, while IronClaw registers custom WASM channels at `/webhook/{channel_name}`. Version `1.7` aligned source, capabilities, and bundle metadata on `/webhook/simon_telegram_channel`; `1.8` adds hosted diagnostics for the remaining admission/identity gap.
